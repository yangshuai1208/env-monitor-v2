...
static uint8_t s_oled_addr = OLED_SLAVER_ADDR;
...
{
    if (HAL_I2C_IsDeviceReady(&hi2c1, (uint16_t)(slave_addr << 1), 2, IIC_TIMEOUT) == HAL_OK)
    {
        return SUCCESS;
    }
...
ErrorStatus OLED_WriteByte(uint8_t cmd, uint8_t byte)
{
    ...
    if (HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)(s_oled_addr << 1), buf, 2, IIC_TIMEOUT) == HAL_OK)
...
ErrorStatus OLED_WriteBuffer(uint8_t cmd, uint8_t *buffer, uint32_t num)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)(s_oled_addr << 1), buf, (uint16_t)(num + 1), IIC_TIMEOUT);
...
{
    static const uint8_t oled_addr_candidates[] = {OLED_SLAVER_ADDR, 0x3D};
    HAL_Delay(100);  // 延时保证电源稳定

    for (uint32_t index = 0; index < OLED_ARRAY_SIZE(oled_addr_candidates); index++)
    {
        for (uint32_t retry = 0; retry < 10; retry++)
        {
            if (OLED_CheckDevice(oled_addr_candidates[index]) == SUCCESS)
            {
                s_oled_addr = oled_addr_candidates[index];
                goto oled_ready;
            }
            HAL_Delay(20);
        }
