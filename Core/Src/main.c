...
/* USER CODE BEGIN PV */
volatile uint8_t g_oled_ready = 0U;					
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	DWT_Init();
  DHT11_GPIO_Config();
	if(AppConfig_Load(&g_app_config)==0)
	{
		char msg[]="CONFIG LOADED\r\n";
		HAL_UART_Transmit(&huart1,(uint8_t*)msg,strlen(msg),100);
	}
	else
	{
		AppConfig_SetDefault(&g_app_config);
		AppConfig_Save(&g_app_config);
		char msg[]="CONFIG DEFAULT SAVED\r\n";
		HAL_UART_Transmit(&huart1,(uint8_t*)msg,strlen(msg),100);
	}
